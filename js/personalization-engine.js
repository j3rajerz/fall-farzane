...
(function (global) {
  ...
  /**
   * نرمال‌سازی نام برای استفاده در بذر (حذف فاصله‌های اضافه، یکسان‌سازی حروف بزرگ/کوچک لاتین).
   */
  ...
  /**
   * نگاشت نمادین ماه تولد شمسی به برج‌های دوازده‌گانه — بخش «افزودن لایهٔ اختربینی».
   * چون هر ماه شمسی از یک اعتدال/انقلاب آغاز می‌شود، تناظرش با برج‌های غربی تقریباً دقیق است
   * (فروردین ≈ حَمَل، اردیبهشت ≈ ثَور، ...). این یک لایهٔ نمادین اضافه است، نه ادعای علمی.
   */
  const ZODIAC_BY_MONTH = [
    { month: 1, name_fa: "بره (حَمَل)", glyph: "♈", element: "آتش" },
    { month: 2, name_fa: "گاو (ثَور)", glyph: "♉", element: "خاک" },
    { month: 3, name_fa: "دوپیکر (جَوزا)", glyph: "♊", element: "باد" },
    { month: 4, name_fa: "خرچنگ (سَرطان)", glyph: "♋", element: "آب" },
    { month: 5, name_fa: "شیر (اَسَد)", glyph: "♌", element: "آتش" },
    { month: 6, name_fa: "خوشه (سُنبله)", glyph: "♍", element: "خاک" },
    { month: 7, name_fa: "ترازو (میزان)", glyph: "♎", element: "باد" },
    { month: 8, name_fa: "کژدم (عَقرب)", glyph: "♏", element: "آب" },
    { month: 9, name_fa: "کمان (قوس)", glyph: "♐", element: "آتش" },
    { month: 10, name_fa: "بزغاله (جَدی)", glyph: "♑", element: "خاک" },
    { month: 11, name_fa: "دلو (دَلو)", glyph: "♒", element: "باد" },
    { month: 12, name_fa: "ماهی (حوت)", glyph: "♓", element: "آب" }
  ];

  function zodiacForMonth(month) {
    return ZODIAC_BY_MONTH[(Math.trunc(month) - 1 + 12) % 12] || ZODIAC_BY_MONTH[0];
  }

  /**
   * ساخت بذر پایهٔ تاریخ تولد. ورودی: {year, month, day}
   * اگر «نام» هم داده شود، در بذر ترکیب می‌شود؛ این‌طوری دو نفر با تاریخ تولد
   * یکسان (مثلاً وقتی برای شخص دیگری فال گرفته می‌شود) چیدمان یکسانی نمی‌گیرند.
   * وقتی نام داده نشود، رفتار دقیقاً مثل قبل (فقط بر پایهٔ تاریخ) باقی می‌ماند.
   */
  function birthSeed(date, name) {
   ...
   *
   * options.avoidIds: شناسهٔ کارت‌هایی که در فال‌های اخیرِ همین شخص آمده‌اند و
   * ترجیحاً باید این بار نیایند (برای کاهش حس «تکراری‌بودن»). اگر بعد از کنارگذاشتن
   * آن‌ها کارت کافی برای این چیدمان نماند، به‌طور خودکار به کل دسته برمی‌گردیم.
   */
  function drawUniqueCards(seed, count, options) {
    options = options || {};
    const fullPool = options.pool || global.TarotEngine.getAllCards();
    const avoidIds = options.avoidIds && options.avoidIds.length ? new Set(options.avoidIds) : null;

    let pool = fullPool;
    if (avoidIds && avoidIds.size) {
      const filtered = fullPool.filter((c) => !avoidIds.has(c.id));
