...
(function () {
  ...
  // ---------------------------------------------------------------
  // کمک‌تابع‌های شخصی‌سازی: شناسهٔ پایدار هر شخص + جلوگیری از تکرار کارت‌های اخیر
  // ---------------------------------------------------------------

  // شناسهٔ پایدار برای «خودم» در برابر هر شخص دیگر (person.id) — برای پیوند فال‌ها به تاریخچهٔ همان شخص
  function personKeyFor(entity, isSelf) {
    return isSelf ? "self" : (entity && entity.id ? entity.id : null);
  }

  // آخرین کارت‌های ظاهرشده برای همین شخص را از تاریخچه جمع می‌کند تا در فال بعدی کمتر تکرار شوند
  function recentCardIdsFor(personKey, lookback) {
    if (!personKey) return [];
    const history = ST.getHistory();
    const ids = [];
    let matched = 0;
    for (let i = 0; i < history.length && matched < (lookback || 2); i += 1) {
      const entry = history[i];
      if (entry.personKey !== personKey || !Array.isArray(entry.cards)) continue;
      matched += 1;
      entry.cards.forEach((c) => { if (c.cardId) ids.push(c.cardId); });
    }
    return ids;
  }
    ...
    if (profile) {
      dailyBlock.innerHTML = `
        <div class="glass-panel" style="margin-bottom:20px;display:flex;align-items:center;gap:14px;">
          ...
          <div style="flex:1;">
            <div style="font-weight:700;font-size:14px;">سلام${profile.name ? " " + escapeHtml(profile.name) : ""}، آماده‌ای فال امروزت را ببینی؟</div>
  ...
  function openBirthDateFlow(onDone) {
    const existing = ST.getProfile();
    UI.openJalaliDatePicker((picked) => {
      if (!JD.isValidJalaliDate(picked.year, picked.month, picked.day)) {
        alert("تاریخ واردشده معتبر نیست.");
        return;
      }
      const date = { year: picked.year, month: picked.month, day: picked.day };
      const name = picked.name || "";
      const numerology = PE.computeBirthProfile(date, name);
      const profile = { date, name, numerology, createdAt: new Date().toISOString() };
      ST.saveProfile(profile);
      playBirthCeremony(profile, onDone);
    }, {
      title: "تاریخ تولد خودت",
      askName: true,
      nameLabel: "اسمت (اختیاری — برای شخصی‌سازی بیشتر فال)",
      initialName: existing && existing.name ? existing.name : ""
    });
  }

  // فال‌گیری برای شخص دیگر — تاریخ تولد کاربر خودمان دست‌نخورده باقی می‌ماند
  // و پروفایل تازه فقط در لیست «افراد» ذخیره می‌شود (بخش رفع اشکال: دیگر
  // فال برای دیگران به‌صورت خاموش از تاریخ تولد قبلی استفاده نمی‌کند).
  function openGuestBirthFlow(onDone) {
    UI.openJalaliDatePicker((picked) => {
      if (!JD.isValidJalaliDate(picked.year, picked.month, picked.day)) {
        alert("تاریخ واردشده معتبر نیست.");
        return;
      ...
      const person = {
        ...
        date, name, numerology, createdAt: new Date().toISOString()
      };
      ST.savePerson(person);
      onDone(person);
    }, {
      title: "تاریخ تولد شخص دیگر",
      askName: true,
      nameRequired: true,
      nameLabel: "اسم این شخص"
    });
  }
  ...
  function renderProfileScreen() {
    ...
    const screen = screenEl(`
      ...
      <div class="glass-panel" style="text-align:center;margin-bottom:18px;">
        ...
        ${profile.numerology.zodiac ? `<div style="font-size:13px;color:var(--text-secondary);margin-bottom:6px;">${profile.numerology.zodiac.glyph} برج نمادین: ${profile.numerology.zodiac.name_fa} (عنصر ${profile.numerology.zodiac.element})</div>` : ""}
    ...
    screen.querySelector("#btn-birth-reading").addEventListener("click", () => {
      const personKey = personKeyFor(profile, true);
      const reading = RE.buildPersonalizedReading("birth", profile, {
        personKey, avoidCardIds: recentCardIdsFor(personKey)
      });
      lastGeneratedReading = reading;
      ST.addHistoryEntry(reading);
      navigate("#/result/last");
  ...
  function renderFlow(typeId) {
    ...
    const screen = screenEl(`
      ...
      <div style="margin-bottom:18px;">
        <span class="field-label">سطح فال</span>
        <div style="display:flex;gap:8px;">
          <button type="button" class="btn-secondary depth-btn active" data-depth="short" style="flex:1;">کوتاه</button>
          <button type="button" class="btn-secondary depth-btn" data-depth="deep" style="flex:1;">عمیق‌تر</button>
        </div>
      </div>
      ...
      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px;">
        ...
        <button class="glass-card-btn" id="mode-personal">
          <span class="icon-badge">🌙</span>
          <span><span class="label-main">فال شخصی با تاریخ تولد</span><br><span class="label-sub">برای خودت یا شخص دیگر، با تاریخ تولد</span></span>
        </button>
      </div>
    `);

    let depth = "short";
    screen.querySelectorAll(".depth-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        depth = btn.dataset.depth;
        screen.querySelectorAll(".depth-btn").forEach((b) => b.classList.toggle("active", b === btn));
      });
    });
    ...
    function runReading(mode) {
      ...
      if (mode === "personal") {
        const people = ST.getPeople();

        // بار اول: هنوز نه تاریخ تولد خودمان ثبت شده، نه شخص دیگری — تاریخ تولد خودمان را می‌گیریم
        if (!profile && people.length === 0) {
          openBirthDateFlow(() => {
            const freshProfile = ST.getProfile();
            const personKey = personKeyFor(freshProfile, true);
            finishReading(RE.buildPersonalizedReading(typeId, freshProfile, {
              question, depth, personKey, avoidCardIds: recentCardIdsFor(personKey)
            }));
          });
          return;
        }

        // در غیر این صورت، همیشه از کاربر می‌پرسیم این فال برای کیست —
        // به‌جای این‌که خاموش از آخرین تاریخ تولد ثبت‌شده استفاده کند.
        UI.openPersonChooserSheet({
          selfProfile: profile,
          people,
          onSelf: () => {
            const personKey = personKeyFor(profile, true);
            finishReading(RE.buildPersonalizedReading(typeId, profile, {
              question, depth, personKey, avoidCardIds: recentCardIdsFor(personKey)
            }));
          },
          onPerson: (person) => {
            const personKey = personKeyFor(person, false);
            finishReading(RE.buildPersonalizedReading(typeId, person, {
              question, depth, personKey, avoidCardIds: recentCardIdsFor(personKey)
            }));
          },
          onNewPerson: () => {
            openGuestBirthFlow((person) => {
              const personKey = personKeyFor(person, false);
              finishReading(RE.buildPersonalizedReading(typeId, person, {
                question, depth, personKey, avoidCardIds: recentCardIdsFor(personKey)
              }));
          ...
          onRemovePerson: (id) => { ST.removePerson(id); }
        });
        return;
      }

      finishReading(RE.buildRandomReading(typeId, { question, depth }));
    }
  ...
  function renderResult(param) {
    ...
    const screen = screenEl(`
      ...
      ${reading.personName ? `<p style="text-align:center;font-size:12px;color:var(--text-muted);margin:6px 0;">این فال برای «${escapeHtml(reading.personName)}» است</p>` : ""}
